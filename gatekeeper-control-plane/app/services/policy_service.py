...
from app.models import DevicePostureRule, Role, RoutePolicy
...
async def create_or_update_policy(
    ...
    # Check if exists
    result = await session.execute(select(RoutePolicy).where(RoutePolicy.name == name))
    ...
    fresh = await session.execute(select(RoutePolicy).where(RoutePolicy.id == policy_id))
...
async def sync_policies_to_redis(session: AsyncSession, redis: aioredis.Redis) -> None:
    ...
    result = await session.execute(select(RoutePolicy).options(selectinload(RoutePolicy.roles)))
...
async def list_all_posture_rules(session: AsyncSession) -> Sequence[DevicePostureRule]:
    result = await session.execute(select(DevicePostureRule).order_by(DevicePostureRule.id.asc()))
    return result.scalars().all()
...
async def create_or_update_posture_rule(
    ...
    result = await session.execute(
        select(DevicePostureRule).where(
            DevicePostureRule.rule_type == rule_type, DevicePostureRule.value == value
        )
    )
...
async def sync_posture_to_redis(session: AsyncSession, redis: aioredis.Redis) -> None:
    ...
    for r in active_rules:
        ...
        if r.action == "block" and r.rule_type in payload:
